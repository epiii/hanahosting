var dir='server/p_mkecamatan.php';
	...
	function loadData(){
		...
		$.ajax({
			...
			data: datax+cari,
			success:function(data){
				$('#loadarea').html('<i class="icon-th-list"></i> DAFTAR mkecamatan');
		...
		$.ajax({
			...
			success:function(data){
				if(data.status=='gagal'){
					...
					kosongkan();
					var id_mkecamatany		= data.id_mkecamatan;
					var goly		= data.gol;
					
					$('#idformTB').val(id); 
					$('#id_mkotaTB').val(id_mkecamatany);
					$('#golTB').val(goly);
					
					combomkecamatan(id_mkecamatany);

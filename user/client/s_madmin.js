var dir='server/p_madmin.php';
	...
	function loadData(){
		...
		var cari ='&namaS='+$('#namaTS').val()
				 ...
				 +'&jabatanS='+$('#jabatanTS').val();
		$.ajax({
			url	: dir,
			type: 'GET',
			data: datax+cari,
			success:function(data){
				$('#loadarea').html('<i class="icon-th-list"></i> DAFTAR Admin');
		...
		$.ajax({
			...
			success:function(data){
				if(data.status=='gagal'){
					...
					$('#notelTB').val(data.no_telp);
					$('#jabatanTB').val(data.jabatan);
					
					combomadmin(data.id_mlogin);
					// combomadmin(id_mlogin);

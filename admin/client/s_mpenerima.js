		...
		$.ajax({
			...
			success:function(data){
				if(data.status=='gagal'){
					...
					$('#malamatwTB').val(data.id_malamatw);
					$('#kodeposwTB').val(data.kode_pos);
					
					$('#id_mprowTB').val(data.id_mpropinsiw);
					$('#id_mkotwTB').val(data.id_mkotaw);
					$('#id_mkecwTB').val(data.id_mkecamatanw);
					
					// combomkecamatan(data.id_mkecamatan);
					// combompropinsi(data.id_mpropinsi);
					// combomkota(data.id_mkota);
					
					combompropinsi('#id_mproppTB',data.id_mpropinsi);
					combompropinsi('#id_mpropoTB',data.id_mpropinsi);
					combompropinsi('#id_mpropwTB',data.id_mpropinsi);

					combomkota('#id_mkotpTB',data.id_mkota);
					combomkota('#id_mkotoTB',data.id_mkota);
					combomkota('#id_mkoawTB',data.id_mkota);

					combomkecamatan('#id_mkecpTB',data.id_mkecamatan);
					combomkecamatan('#id_mpkecoTB',data.id_mkecamatan);
					combomkecamatan('#id_mkecwTB',data.id_mkecamatan);
...
//end of fungsi tooltip________________________________________________________
	...
	function tmbhsekolah () {
		tr='<tr id="tr">'
				// +'<form id="lokasiFR" onSubmit="return simpanlokasi(this);">'
					...
					+'<td>'
						+'<div class="btn-group">'
						  	+'<button onclick="simpansekolah();" class="btn btn-info" onmouseover="return tooltipx(this);" ' 
						  		+'data-toggle="tooltip" title="simpan" data-placement="top" >'
						  		+'<i class="icon-ok"></i>'
					  		+'</button>'
							+'<button class="btn btn-danger" onclick="hapussekolah(\'\');" onmouseover="return tooltipx(this);" ' 
			...
			success:function(data){
				...
				if(data.status!='sukses'){
					...
					$.each(data.datax,function(id,item){
						...
						tb+='<tr '+tr+' onmouseover="return tooltipx(this);" data-toggle="tooltip" title="'+trinfo+'" data-placement="top">'
								+'<td>'+no+'</td>'
								+'<td>'+item.nama+'</td>'
								...
								+'<td>'
									+'<div class="btn-group">'
									  	+'<button '+dis+' onclick="statsekolah('+id_mpenerima+','+item.id_dpenerima+');" class="btn btn-'+btn+'" onmouseover="return tooltipx(this);" ' 
									  		+'data-toggle="tooltip" title="'+btninfo+'" data-placement="top" >'
									  		+'<i class="icon-'+icon+'"></i>'
								  		+'</button>'
										+'<button '+dis+' class="btn btn-'+btn2+'" onclick="hapussekolah('+item.id_dpenerima+');" onmouseover="return tooltipx(this);" ' 
									  		+'data-toggle="tooltip" title="hapus" data-placement="top" >'
											+'<i class="icon-remove"></i>'
										+'</button>'
									+'</div>'
	...
	function simpansekolah(){
		if ($('#id_msekolahTB').val()=='') {
			...
			datax 	='id_mpenerima='+$('#id_mpenerimaH').val()
					+'&id_msekolah='+$('#id_msekolahTB').val()
					+'&id_mlembaga='+$('#id_mlembagaTB').val();
	...
	function statsekolah (id_mpenerima,id_dpenerima) {
		$.ajax({
			url:dir,
			cache:false,
			data:'aksi=cek&id_mpenerima='+id_mpenerima+'&id_dpenerima='+id_dpenerima,
			dataType:'json',
			success:function(data){
				if(data.status!='sukses'){
					alert(data.status);
				}else{
					viewsekolah(id_mpenerima);
				}
			}
		});
	}

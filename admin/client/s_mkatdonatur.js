var dir = 'server/p_mkatdonatur.php';
	function loadData(){
		...
		$.ajax({
			...
			success:function(data){
				$('#loadarea').html('<i class="icon-th-list"></i> KATEGORI DONATUR');
	...
	function submitForm(event){
		...
		var id_mkatdonatur = +$('#idformTB').val()
		var urlx =dir+'?';

		if($('#idformTB').val()==''){ //add
			urlx += 'aksi=tambah';
		}else{ //edit
			urlx += 'aksi=ubah&id_mkatdonatur='+id_mkatdonatur;
		}
	...
	function hapusmkatdonatur(id){
		if(confirm('melanjutkan untuk menghapus data?'))
		$.ajax({
			url:dir,
			type:'get',
			data:'aksi=hapus&id_mkatdonatur='+id,
			dataType:'json',
			success:function(data){
	...
	function editmkatdonatur(id){
		//alert('id: '+id);
		$.ajax({
			url:dir,
			type:'get',
			data:'aksi=ambiledit&id_mkatdonatur='+id,
			dataType:'json',
			success:function(data){
				if(data.status=='gagal'){
					...
					$('#idformTB').val(id); 
					$('#mdonaturTB').val(data.mkatdonatur);
